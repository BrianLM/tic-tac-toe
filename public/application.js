...
var onGetGame = function onGetGame(id) {
  return $.ajax({
...
var onJoinGame = function onJoinGame(id) {
  return $.ajax({
    url: config.apiOrigin + '/games/' + id,
    method: 'PATCH',
    headers: {
...
var onInvite = function onInvite() {
...
var cellClick = function cellClick(event) {
  var cellIndex = +this.attributes['data-index'].value;
  var cell = $('div[data-index="' + cellIndex + '"]');
  var currentPlayer = $('#gameboard').attr('data-player');
  var playingAs = $('#player').attr('data-tag');
  var haveOpponent = $('#opponent').attr('data-tag');
  var data = { 'game': { 'cell': { 'index': cellIndex, 'value': currentPlayer }, 'over': 'false' } };
  if (cell.attr('data-move') === '' && !game.game.over && (haveOpponent === 'none' || currentPlayer === playingAs)) {
    sendGameUpdate(data);
  }
};

var sendGameUpdate = function sendGameUpdate(data) {
  gameAPI.updateGame(data).then(gameUI.onMoveSuccess).catch(gameUI.onMoveFailure);
};
...
var startNewGame = function startNewGame(event) {
  ...
  var gameInPlay = $('#playarea').attr('data-state');
  if (Object.keys(game).length === 0 || gameInPlay === 'true') {
    event.preventDefault();
    gameAPI.startNewGame().then(gameUI.onCreateSuccess).catch(gameUI.onCreateFailure);
  } else {
    switch (event.target.id) {
      case 'forfeit-game':
        {
          var data = { 'game': { 'cell': { 'index': 0, 'value': game.game.cells[0] }, 'over': 'true' } };
          gameAPI.updateGame(data).then(gameUI.onFinishSuccess).catch(gameUI.onFinishFailure).then(gameAPI.startNewGame).then(gameUI.onCreateSuccess).catch(gameUI.onCreateFailure);
          break;
        }
      case 'save-game':
        {
...
var getGameByID = function getGameByID(event) {
  event.preventDefault();
  var data = getFormFields(this);
  gameAPI.onGetGame(data.game.id).then(gameUI.onGetSuccess).catch(gameAPI.onJoinGame(data.game.id).then(gameUI.onGetSuccess).catch(gameUI.onGetFailure));
};

var getGamesByUser = function getGamesByUser(event) {
  event.preventDefault();
  gameAPI.listGames().then(gameUI.onListSuccess).catch(gameUI.onListFailure).then($('#list-modal').modal('show'));
};

var getSelectedGame = function getSelectedGame(event) {
  event.preventDefault();
  var selectedID = event.target.attributes['data-gameid'].value;
  gameAPI.onGetGame(selectedID).then(gameUI.onGetSuccess).catch(gameUI.onGetFailure);
  $('#list-modal').modal('hide');
};

var getStatistics = function getStatistics(event) {
  event.preventDefault();
  gameAPI.getAllGames().then(gameUI.onStatsSuccess).catch(gameUI.onStatsFailure);
};

var aiTurn = function aiTurn(event) {
  var available = game.game.cells.map(function (current, index) {
    return current === '' ? index : '';
  }).filter(function (x) {
    return x !== '';
  });
  var aiClick = Math.round(Math.random() * (available.length - 1));
  var data = { 'game': { 'cell': { 'index': available[aiClick], 'value': 'o' }, 'over': 'false' } };
  sendGameUpdate(data);
};

var toggleAI = function toggleAI(event) {
  event.preventDefault();
  ...
  if ($('#gameboard').attr('data-player') === 'o') {
    var data = { 'game': { 'cell': { 'index': 0, 'value': game.game.cells[0] }, 'over': 'false' } };
    sendGameUpdate(data);
  }
};
...
var store = __webpack_require__(3);
var game = __webpack_require__(1);
var conditions = __webpack_require__(16);
var config = __webpack_require__(2);
...
var onInviteRequest = function onInviteRequest() {
  var gameWatcher = watcher.resourceWatcher(config.apiOrigin + '/games/' + game.game.id + '/watch', {
    Authorization: 'Token token=' + store.user.token
  });
...
var onMoveSuccess = function onMoveSuccess(response, status, xhr) {
  game.game = response.game;
  conditions.setTiles();
};

var onMoveFailure = function onMoveFailure(response, status, xhr) {};

var onFinishSuccess = function onFinishSuccess(response, status, xhr) {};

var onFinishFailure = function onFinishFailure(response, status, xhr) {};

var onGetSuccess = function onGetSuccess(response, status, xhr) {
  game.game = response.game;
  var online = game.game['player_o'] !== null;
  $('#input-join').val('');
  conditions.setTiles();
  $('#playarea').removeClass('hidden');
  $('#list-modal').modal('hide');
...
var onListSuccess = function onListSuccess(response, status, xhr) {
  game.games = response.games;
  var listGameArea = $('#listContent');
  listGameArea.empty();
  var gameAreaForm = $('<form/>');
  if (game.games.count === undefined) {
    gameAreaForm.append('<p>No games found that can be played.</p>');
  }
  game.games.forEach(function (current, index, array) {
...
var onListFailure = function onListFailure(response, status, xhr) {};

var onStatsSuccess = function onStatsSuccess(response, status, xhr) {
  game.games = response.games;
  var win = 0;
  var loss = 0;
  var tie = 0;
  game.games.forEach(function (current, index, array) {
    var result = conditions.getGameWinner(current.cells);
    if (result === 0) {
      tie++;
...
var onStatsFailure = function onStatsFailure(response, status, xhr) {};
...
module.exports = {
  ...
  onFinishFailure: onFinishFailure,
  onListSuccess: onListSuccess,
  onListFailure: onListFailure,
...
var setTiles = function setTiles() {
  var state = game.game.cells;
  state.forEach(function (current, index) {
    var cell = $('div[data-index="' + index + '"]');
    if (current === 'x') {
  ...
  var xMoves = state.filter(function (x) {
    return x === 'x';
  });
  var oMoves = state.filter(function (o) {
    return o === 'o';
  });
  var some = state.some(function (x) {
    return x !== '';
  });
  if (some) {
    checkWinCondition();
...
var checkWinCondition = function checkWinCondition() {
  var state = game.game.cells;
  var tie = state.every(function (x) {
    return x !== '';
  });
...
var turnIndicator = function turnIndicator(currentPlayer) {
  if (currentPlayer === 'x') {
...
var finishTurn = function finishTurn(currentPlayer) {
  var board = $('#gameboard');
  if (currentPlayer === 'x') {
...
var getGameWinner = function getGameWinner(array) {
  var tie = array.every(function (x) {
    return x !== '';
  });
